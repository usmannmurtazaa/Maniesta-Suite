...
export default function CalculatorPanel({ darkMode }) {
  ...
  return (
    <div ref={containerRef} className="max-w-lg mx-auto w-full space-y-4">
      ...
      {mode === "scientific" && (
        <div className="flex gap-2" role="radiogroup" aria-label="Angle unit">
          {["deg", "rad"].map((m) => (
            <button
              key={m}
              onClick={() => {
      ...
      {/* Memory indicator */}
      {memory !== 0 && (
        <div className="inline-block px-3 py-1 rounded-full bg-purple-500/10 text-purple-400 text-xs font-mono font-medium backdrop-blur-sm">
          M: {memory}
        </div>
      )}
      ...
      {/* Display */}
      <div
        className={`p-5 rounded-2xl backdrop-blur-2xl border min-h-[6rem] flex flex-col justify-end ${
          isDark
            ? "bg-indigo-950/60 border-white/10 shadow-xl shadow-black/30"
            : "bg-white/70 border-black/5 shadow-lg"
        }`}
        aria-live="polite"
        aria-atomic="true"
      >
        <div className="text-sm text-right text-gray-400 dark:text-gray-500 font-mono break-all min-h-[1.5rem]">
          {input || "0"}
        </div>
          ...
          row.map((btn, j) => {
            ...
            return (
              <motion.button
                ...
                className={`py-3 rounded-xl text-lg font-medium transition-all backdrop-blur-sm select-none ${
                  isAction
                    ? "bg-gradient-to-r from-purple-600 to-purple-800 text-white shadow-md shadow-purple-500/20"
                    : isMemory
                      ? "bg-purple-500/15 border border-purple-500/20 text-purple-600 dark:text-purple-300"
