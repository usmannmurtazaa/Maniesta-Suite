...
/**
 * Normalize different data shapes into a consistent object.
 * Supports both old ExportModal call and new direct call.
 */
function normalizeExportData(data) {
  if (data.userData) {
    // Old ExportModal call: { userData, resultData, calculatorType }
    const { userData, resultData, calculatorType } = data;
    const isCGPA = calculatorType === 'CGPA';
    const courses = !isCGPA ? (resultData.courses || []).map(c => ({
      code: c.name,
      credits: c.creditHours,
      grade: c.grade,
      points: (gradePoints?.[c.grade] || 0) * (parseFloat(c.creditHours) || 0),
    })) : [];
    const semesters = isCGPA ? (resultData.semesters || []).map(sem => ({
      gpa: sem.courses ? (sem.courses.reduce((acc, c) => acc + (gradePoints?.[c.grade] || 0) * parseFloat(c.creditHours), 0) / (sem.courses.reduce((acc, c) => acc + parseFloat(c.creditHours), 0) || 1)).toFixed(2) : '0',
    ...
    return {
      ...
      courses,
      semesters,
      gpaResult: isCGPA ? {} : { gpa: resultData.gpa, credits: courses.reduce((acc, c) => acc + c.credits, 0), points: courses.reduce((acc, c) => acc + (c.points || 0), 0).toFixed(2) },
      cgpaResult: isCGPA ? { cgpa: resultData.cgpa, total: semesters.reduce((acc, s) => acc + parseFloat(s.gpa), 0), best: Math.max(...semesters.map(s => parseFloat(s.gpa))) } : {},
    };
  }
  // New direct call (used by CGPACalculator): { studentName, studentId, ..., type, semesters, cgpaResult, ... }
  return {
...
// Import gradePoints to calculate points in normalization
...
export async function generatePDF(data) {
  try {
    ...
    const sanitized = (normalized.studentName || 'Student').replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_]/g, '');
...
export function generateCSV(data, type) {
  ...
  const lines = [];
  lines.push('MANIESTA CALCULATOR - ' + (isCGPA ? 'CGPA REPORT' : 'ACADEMIC RECORD'));
  lines.push('==========================================');
  lines.push('');
  ...
  lines.push('');
  lines.push('Generated by Maniesta Calculator - Crafted by Usman Murtaza');
  lines.push('maniestacalculator.com');
  return '\uFEFF' + lines.join('\n');
}
